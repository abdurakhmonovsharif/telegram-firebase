...
const CountryCode = (props) => {
    ...
    const { set } = props
    return (
        <div style={{ width: 320 + "px", marginTop: 20 + "px", textAlign: 'left' }}>
            <Select placeholder="search your country"
                onChange={(value) => set(value.value)}
                options={countries.map(item => ({ value: item.mobileCode, label: item.mobileCode + " " + item.name }))}
            />
        </div>
    );
}
